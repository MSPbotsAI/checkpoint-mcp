...
from checkpoint_mcp.__main__ import _build_http_app
from checkpoint_mcp.config import Settings
from checkpoint_mcp.server import create_mcp_server, get_client_from_context
...
def test_missing_header_returns_401_with_required_headers_listed():
    ...
    with TestClient(app) as client:
        ...
        body = resp.json()
        assert "X-CheckPoint-Client-Id" in body["required_headers"]
        assert "X-CheckPoint-Access-Key" in body["required_headers"]
        assert "X-CheckPoint-Region" in body["required_headers"]
    ...
    with TestClient(app) as client:
        resp = client.post(
            ...
            headers={
                ...
                "X-CheckPoint-Client-Id": "cid",
                # Access-Key and Region missing.
            },
        )
        assert resp.status_code == 401
...
def test_header_present_reaches_request_context(monkeypatch):
    ...
    from checkpoint_mcp.server import GatewayTokenMiddleware, _gateway_creds_var
    ...
    async def run():
        scope = {
            ...
            "path": "/mcp",
            "headers": [
                (b"x-checkpoint-client-id", b"test-client-id"),
                (b"x-checkpoint-access-key", b"test-access-key"),
                (b"x-checkpoint-region", b"https://cloudinfra-gw.portal.checkpoint.com"),
            ],
        }
